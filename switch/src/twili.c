...
static int _twiliDotabOpen(struct _reent *r, void *fdptr, const char *path,
                           int flags, int mode);
static int _twiliDotabClose(struct _reent *r, void *fdptr);
static ssize_t _twiliDotabWrite(struct _reent *r, void *fdptr, const char *buf,
                                size_t count);
static ssize_t _twiliDotabRead(struct _reent *r, void *fdptr, char *buf,
                               size_t count);
...
Result twiliCreateExactlyNamedOutputPipe(TwiliPipe *pipe, const char *name,
                                         size_t len) {
  return serviceDispatch(
      &g_twiliSrv, 10, .out_num_objects = 1, .out_objects = &pipe->_srv,
      .buffer_attrs = {SfBufferAttr_In | SfBufferAttr_HipcMapAlias},
      .buffers = {{name, len}});
}

static Result _twiliOpenStandardPipeByCommandId(TwiliPipe *pipe, int cmd_id) {
  return serviceDispatch(&g_twiliSrv, cmd_id, .out_num_objects = 1,
                         .out_objects = &pipe->_srv, .in_send_pid = true);
}
...
int twiliBindPipe(TwiliPipe *pipe) {
  ...
  memcpy(__get_handle(fd)->fileStruct, pipe, sizeof(*pipe));
...
int twiliBindStdio() {
  TwiliPipe tp_stdin;
  bool owns_stdin = false;
  TwiliPipe tp_stdout;
  bool owns_stdout = false;
  TwiliPipe tp_stderr;
  bool owns_stderr = false;
  ...
  if (owns_stdin) {
    twiliClosePipe(&tp_stdin);
  }
  if (owns_stdout) {
    twiliClosePipe(&tp_stdout);
  }
  if (owns_stderr) {
    twiliClosePipe(&tp_stderr);
  }
...
Result twiliReadPipe(TwiliPipe *pipe, char *buffer, size_t buffer_length,
                     size_t *read_length) {
  return serviceDispatchOut(
      &pipe->_srv, 0, *read_length,
      .buffer_attrs = {SfBufferAttr_Out | SfBufferAttr_HipcMapAlias},
      .buffers = {{buffer, buffer_length}});
}

void twiliClosePipe(TwiliPipe *pipe) { serviceClose(&pipe->_srv); }
...
static int _twiliDotabOpen(struct _reent *r, void *fdptr, const char *path,
  ...
  if (strncmp(path, "twili:", 6) == 0) {
    path += 6;
  }

  return _twiliAdjustErrno(twiliCreateNamedOutputPipe((TwiliPipe *)fdptr, path),
                           r)
             ? 0
             : -1;
}
...
static ssize_t _twiliDotabWrite(struct _reent *r, void *fdptr, const char *buf,
                                size_t count) {
  return _twiliAdjustErrno(twiliWritePipe((TwiliPipe *)fdptr, buf, count), r)
             ? count
             : -1;
}
...
static ssize_t _twiliDotabRead(struct _reent *r, void *fdptr, char *buf,
                               size_t count) {
  size_t actual_count;
  return _twiliAdjustErrno(
             twiliReadPipe((TwiliPipe *)fdptr, buf, count, &actual_count), r)
             ? actual_count
             : -1;
}
