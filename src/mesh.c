...
uint32_t create_mesh_pipeline(VkDevice device, VkPipelineCache cache,
  ...
  VkPipeline mesh_pipeline = VK_NULL_HANDLE;
  {
    ...
    VkShaderModule frag_mod = VK_NULL_HANDLE;
    {
      ...
      create_info.codeSize = sizeof(mesh_vert);
      create_info.pCode = (const uint32_t *)mesh_vert;
      err = vkCreateShaderModule(device, &create_info, NULL, &vert_mod);
      assert(err == VK_SUCCESS);

      create_info.codeSize = sizeof(mesh_frag);
      create_info.pCode = (const uint32_t *)mesh_frag;
...
#define VB_MAX 3
    VkVertexInputBindingDescription vert_bindings[VB_MAX] = {
...
#define VA_MAX 3
    VkVertexInputAttributeDescription vert_attrs[VA_MAX] = {
    ...
    vert_input_state.vertexBindingDescriptionCount = VB_MAX;
    vert_input_state.pVertexBindingDescriptions = vert_bindings;
    vert_input_state.vertexAttributeDescriptionCount = VA_MAX;
